...
async def websocket_endpoint(websocket: WebSocket,  file_id: str, db: Session = Depends(get_db)):
    ...
    while status != 'completed':
        await websocket.send_json({
            "status": "processing",
            "progress": get_with_fallback("file_processing", file_id, "int", lambda k: get_file_progress(db, k)),
            "total": get_with_fallback("file_total", file_id, "int", lambda k: get_total_records(db, k)),
            "errors": get_with_fallback("row_with_errors", file_id, "int", lambda k: get_error_count(db, k))
        })
